...
class Pointers:
    ...
    def __init__(self, hwnd=None, max_retries=3, retry_delay=1.0):
        ...
        try:
            ...
            self.Y_POINTER = self.get_pointer(self.CLIENT + 0x025A3AF8, offsets=[0xA4])
            ...
            self.HP_POINTER_MAX = self.get_pointer(self.CLIENT + 0x0352D14C, offsets=[0x28]) + 0x8
            self.SD_POINTER = self.get_pointer(self.CLIENT + 0x0352D14C, offsets=[0x38])
            self.SD_POINTER_MAX = self.get_pointer(self.CLIENT + 0x0352D14C, offsets=[0x38]) + 0x4
            self.ZEN_POINTER1 = self.get_pointer(self.CLIENT + 0x0352D14C, offsets=[0xA80])
            self.NOME_CHAR_POINTER = self.get_pointer(self.CLIENT + 0x0352D14C, offsets=[0x0])
            self.PONTO_LVL_POINTER = self.get_pointer(self.CLIENT + 0x0352D14C, offsets=[0x88])
            self.RESET_POINTER = self.get_pointer(self.CLIENT + 0x0352D14C, offsets=[0x10])
            self.LVL_POINTER = self.get_pointer(self.CLIENT + 0x0352D14C, offsets=[0x0]) + 0x0E
            #
            self.MOSTRAR_DESC_POINTER = self.get_pointer(self.CLIENT + 0x0422E2F4, offsets=[0x18])
            #
            self.PK_ATIVO_POINTER = self.get_pointer(self.CLIENT + 0x00113328, offsets=[0x0])
            #
            self.CHAR_PK_SELECIONADO_POINTER = self.get_pointer(self.CLIENT + 0x000D9184, offsets=[0x0])
            self.ITEM_SELECIONADO_POINTER = self.get_pointer(self.CLIENT + 0x000D9184, offsets=[0x4])
            #
            self.SALA_ATUAL_POINTER = self.get_pointer(self.CLIENT + 0x001514AC, offsets=[0x104])
            #
            self.CLASSE_POINTER = self.get_pointer(self.CLIENT + 0x0422DB00, offsets=[0x4C])
            #
            # QUANDO PESQUISAR O POINTER VALIDOR O CODIGO DE TODOS OS MAPAS. A PESQUISA FAÇA PELO CODIGO QUE ESTA EM PATHFINDER
            self.MAPA_ATUAL_POINTER = self.CLIENT + 0x39E2D24
            #
            # pointer_base = self.CLIENT + 0x0352D14C
            # if pointer_base:
    ...
    def teste_pointer_necessarios(self):
        ...
        print('NIVEL PK:' + str(self.get_nivel_pk()))
        ...
        print('ITEM SELECIONADO:' + str(self.get_item_selecionado()))
    ...
    def get_item_selecionado(self):
        valor = int(self.read_value(self.ITEM_SELECIONADO_POINTER, data_type="word"))
        if valor == 65535:
            return False
        return True
    ...
    def imprimir_todos_tipos_do_endereco_memoria(self, endereco_raiz=None, tamanho=0x0B00):
        """
        Lê a estrutura apontada por (CLIENT + 0x0352D14C) e imprime todos os tipos
        em cada offset (BYTE/WORD/DWORD/FLOAT), caminhando byte a byte.

        - endereco_raiz: se None, usa self.CLIENT + 0x0352D14C e dereferencia.
        ...
        try:
            if endereco_raiz is None:
                ...
                ptr_addr = self.CLIENT + 0x0352D14C
            ...
            if not struct_base:
                print("[ERRO] Ponteiro raiz nulo/zero ao dereferenciar 0x0352D14C.")
                return
